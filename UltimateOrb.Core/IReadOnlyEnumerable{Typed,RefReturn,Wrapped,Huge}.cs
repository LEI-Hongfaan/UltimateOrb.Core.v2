
namespace UltimateOrb.Huge.Collections.Generic {
    
    public partial interface IReadOnlyEnumerable<out T>
	    : System.Collections.Generic.IEnumerable<T> {
    }
}

namespace UltimateOrb.Wrapped.Collections.Generic {
    
    public partial interface IReadOnlyEnumerable<out T>
	    : System.Collections.Generic.IEnumerable<T> {
    }
}

namespace UltimateOrb.Wrapped_Huge.Collections.Generic {
    
    public partial interface IReadOnlyEnumerable<out T>
	    : Huge.Collections.Generic.IReadOnlyEnumerable<T>
	    , Wrapped.Collections.Generic.IReadOnlyEnumerable<T> {
    }
}

namespace UltimateOrb.RefReturn.Collections.Generic {
    
    public partial interface IReadOnlyEnumerable<T>
	    : System.Collections.Generic.IEnumerable<T> {
    }
}

namespace UltimateOrb.RefReturn_Huge.Collections.Generic {
    
    public partial interface IReadOnlyEnumerable<T>
	    : Huge.Collections.Generic.IReadOnlyEnumerable<T>
	    , RefReturn.Collections.Generic.IReadOnlyEnumerable<T> {
    }
}

namespace UltimateOrb.RefReturn_Wrapped.Collections.Generic {
    
    public partial interface IReadOnlyEnumerable<T>
	    : Wrapped.Collections.Generic.IReadOnlyEnumerable<T>
	    , RefReturn.Collections.Generic.IReadOnlyEnumerable<T> {
    }
}

namespace UltimateOrb.RefReturn_Wrapped_Huge.Collections.Generic {
    
    public partial interface IReadOnlyEnumerable<T>
	    : Wrapped_Huge.Collections.Generic.IReadOnlyEnumerable<T>
	    , RefReturn_Huge.Collections.Generic.IReadOnlyEnumerable<T>
	    , RefReturn_Wrapped.Collections.Generic.IReadOnlyEnumerable<T> {
    }
}

namespace UltimateOrb.Typed.Collections.Generic {
    
    public partial interface IReadOnlyEnumerable<out T, out TEnumerator>
	    : System.Collections.Generic.IEnumerable<T>
        where TEnumerator: System.Collections.Generic.IEnumerator<T> {
    }
}

namespace UltimateOrb.Typed_Huge.Collections.Generic {
    
    public partial interface IReadOnlyEnumerable<out T, out TEnumerator>
	    : Huge.Collections.Generic.IReadOnlyEnumerable<T>
	    , Typed.Collections.Generic.IReadOnlyEnumerable<T, TEnumerator>
        where TEnumerator: System.Collections.Generic.IEnumerator<T> {
    }
}

namespace UltimateOrb.Typed_Wrapped.Collections.Generic {
    
    public partial interface IReadOnlyEnumerable<out T, out TEnumerator>
	    : Wrapped.Collections.Generic.IReadOnlyEnumerable<T>
	    , Typed.Collections.Generic.IReadOnlyEnumerable<T, TEnumerator>
        where TEnumerator: System.Collections.Generic.IEnumerator<T> {
    }
}

namespace UltimateOrb.Typed_Wrapped_Huge.Collections.Generic {
    
    public partial interface IReadOnlyEnumerable<out T, out TEnumerator>
	    : Wrapped_Huge.Collections.Generic.IReadOnlyEnumerable<T>
	    , Typed_Huge.Collections.Generic.IReadOnlyEnumerable<T, TEnumerator>
	    , Typed_Wrapped.Collections.Generic.IReadOnlyEnumerable<T, TEnumerator>
        where TEnumerator: System.Collections.Generic.IEnumerator<T> {
    }
}

namespace UltimateOrb.Typed_RefReturn.Collections.Generic {
    
    public partial interface IReadOnlyEnumerable<T, out TEnumerator>
	    : RefReturn.Collections.Generic.IReadOnlyEnumerable<T>
	    , Typed.Collections.Generic.IReadOnlyEnumerable<T, TEnumerator>
        where TEnumerator: RefReturn.Collections.Generic.IReadOnlyEnumerator<T> {
    }
}

namespace UltimateOrb.Typed_RefReturn_Huge.Collections.Generic {
    
    public partial interface IReadOnlyEnumerable<T, out TEnumerator>
	    : RefReturn_Huge.Collections.Generic.IReadOnlyEnumerable<T>
	    , Typed_Huge.Collections.Generic.IReadOnlyEnumerable<T, TEnumerator>
	    , Typed_RefReturn.Collections.Generic.IReadOnlyEnumerable<T, TEnumerator>
        where TEnumerator: RefReturn.Collections.Generic.IReadOnlyEnumerator<T> {
    }
}

namespace UltimateOrb.Typed_RefReturn_Wrapped.Collections.Generic {
    
    public partial interface IReadOnlyEnumerable<T, out TEnumerator>
	    : RefReturn_Wrapped.Collections.Generic.IReadOnlyEnumerable<T>
	    , Typed_Wrapped.Collections.Generic.IReadOnlyEnumerable<T, TEnumerator>
	    , Typed_RefReturn.Collections.Generic.IReadOnlyEnumerable<T, TEnumerator>
        where TEnumerator: RefReturn.Collections.Generic.IReadOnlyEnumerator<T> {
    }
}

namespace UltimateOrb.Typed_RefReturn_Wrapped_Huge.Collections.Generic {
    
    public partial interface IReadOnlyEnumerable<T, out TEnumerator>
	    : RefReturn_Wrapped_Huge.Collections.Generic.IReadOnlyEnumerable<T>
	    , Typed_Wrapped_Huge.Collections.Generic.IReadOnlyEnumerable<T, TEnumerator>
	    , Typed_RefReturn_Huge.Collections.Generic.IReadOnlyEnumerable<T, TEnumerator>
	    , Typed_RefReturn_Wrapped.Collections.Generic.IReadOnlyEnumerable<T, TEnumerator>
        where TEnumerator: RefReturn.Collections.Generic.IReadOnlyEnumerator<T> {
    }
}
