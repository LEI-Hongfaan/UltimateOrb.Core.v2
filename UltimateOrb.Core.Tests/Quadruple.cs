...
namespace UltimateOrb {
    ...
    public readonly struct Quadruple : IEquatable<Quadruple> {
        ...
        public static Quadruple operator +(Quadruple first, Quadruple second) {
            var first_lo = first._Lo64Bits;
            var first_hi = first._Hi64Bits;
            var first_s = Binary128Arithmetic.GetRawSignFromHi64Bits(first_hi);
            var second_lo = second._Lo64Bits;
            var second_hi = second._Hi64Bits;
            var second_s = Binary128Arithmetic.GetRawSignFromHi64Bits(second_hi);
            if (first_s == second_s) {
                return new Quadruple(Binary128Arithmetic.AddAbsoluteValuesThenCopySign(first_lo, first_hi, second_lo, second_hi, first_s, FloatingPointRounding.ToNearestWithMidpointToEven, out var t), t);
            } else {
                return new Quadruple(Binary128Arithmetic.SubtractAbsoluteValuesThenCopySign(first_lo, first_hi, second_lo, second_hi, first_s, FloatingPointRounding.ToNearestWithMidpointToEven, out var t), t);
            }
        }
        ...
        public static Quadruple operator *(Quadruple first, Quadruple second) {
            ...
            if (0x7FFF000000000000 != first_e) {
                if (0x7FFF000000000000 != second_e) {
                    var first_lo = first.RawFractionLo;
                    var first_hi = first.RawFractionHi;
                    var e = -0x3FFF;
                    if (0 != first_e) {
                        ...
                        var first_lzc = BinaryNumerals.CountLeadingZeros(first_hi);
                        if (64 == first_lzc) {
                            unchecked {
                                first_lzc += BinaryNumerals.CountLeadingZeros(first_lo);
                            }
                        }
                        unchecked {
                            e += (FractionBitCount - 64) - first_lzc;
                        }
                        first_lo = DoubleArithmetic.ShiftLeft(first_lo, first_hi, unchecked((128 - FractionBitCount) + 1 + first_lzc), out first_hi);
                    }

                    var second_lo = second.RawFractionLo;
                    var second_hi = second.RawFractionHi;
                    if (0 != second_e) {
                        second_hi |= Hi64BitsImplicitBit;
                    } else {
                        if (0 == (second_hi | second_lo)) {
                            return new Quadruple(0, 0x8000000000000000 & (first._Lo64Bits ^ second._Lo64Bits));
                        }
                        var second_lzc = BinaryNumerals.CountLeadingZeros(second_hi);
                        if (64 == second_lzc) {
                            unchecked {
                                second_lzc += BinaryNumerals.CountLeadingZeros(second_lo);
                            }
                        }
                        unchecked {
                            e += (FractionBitCount - 64) - second_lzc;
                        }
                        second_lo = DoubleArithmetic.ShiftLeft(second_lo, second_hi, unchecked((128 - FractionBitCount) + 1 + second_lzc), out second_hi);
                    }
                    unchecked {
                        e += (int)((first_e + second_e) >> (FractionBitCount - 64));
                    }
                    ...
                    var ee = unchecked((int)(result_hi_hi >> 33));
                    var prerounding_shift_count = 46;
                    result_hi_hi &= ~(0x0000000100000000u << ee);
                    unchecked {
                        prerounding_shift_count += ee;
                    }
                    unchecked {
                        e += ee;
                    }
                    ...
                    {
                        UInt64 r;
                        var minus_count = unchecked(-prerounding_shift_count);
                        result_hi_hi = (result_hi_hi << (/*63 & */minus_count)) | (result_hi_lo >> prerounding_shift_count);
                        r = result_hi_lo << (/*63 & */minus_count);
                        if (0 != result_lo_hi) {
                            r |= 1;
                        }
                        result_hi_lo = r;
                    }
                    var hi = (0x8000000000000000u & (first._Hi64Bits ^ second._Hi64Bits));
                    if (e > 0) {
                        if (0x7FFF > e) {
                        } else {
                            // Infinity
                            return new Quadruple(0, hi | 0x7FFF000000000000u);
                        }
                    } else {
                        goto L_Sub;
                    }
                L_0:;
                    var a = 1u & (0Xc8U >> (7 & unchecked((int)result_hi_lo)));
                    result_hi_lo = DoubleArithmetic.ShiftRight(result_hi_lo, result_hi_hi, 2, out result_hi_hi);
                    if (0 != a) {
                        result_hi_lo = DoubleArithmetic.IncreaseUnchecked(result_hi_lo, result_hi_hi, out result_hi_hi);
                    }
                    return new Quadruple(result_hi_lo, unchecked((hi | ((UInt64)(uint)e << (FractionBitCount - 64))) + result_hi_hi));
                L_Sub:;
                    {
                        result_hi_hi |= Hi64BitsImplicitBit << 2;
                        var count = unchecked(1 - e);
                        e = 0;
                        if (count < 64) {
                            var minus_count = unchecked(-count);
                            result_hi_lo = result_hi_hi << (/*63 & */minus_count) | result_hi_lo >> count | ((UInt64)(result_hi_lo << (/*63 & */minus_count)) == 0 ? 0u : 1u);
                            result_hi_hi = result_hi_hi >> count;
                        } else {
                            result_hi_lo = (count < 127) ?
                                result_hi_hi >> (/*63 & */count) | (((result_hi_hi & (((UInt64)1 << (/*63 & */count)) - 1)) | result_hi_lo) == 0 ? 0u : 1u) :
                                1u;
                            result_hi_hi = 0;
                        }
                        goto L_0;
